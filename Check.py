...
def PAD_Momentum(input_par, psi, grid, m):
    ...
    x_axis = np.linspace(-2. , 2., 60)
    y_axis = np.linspace(-2. , 2., 60)
    z_axis = np.linspace(-3. , 3., 60)
    ...
    pad_value_total = np.zeros((z_axis.size,y_axis.size, x_axis.size))
    ...
    for i, z in enumerate(z_axis):
        ...
        for j, y in enumerate(y_axis):
            ...
            for k, x in enumerate(x_axis):
                ...
                # pad_value_total[i, j, k] = np.power(np.abs(val),2)
    ...
    np.save("H2_Mag", pad_value_magnitude)
    np.save("H2_Phase", pad_value_phase)
